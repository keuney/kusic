...
KM-056은 재생 요청의 클라이언트 설정을 프로필로 분리해 완료했다. WEB은 직접 오디오 URL 없이 별도 전송 주소만 반환하므로 후보를 순서대로 시도하며, 실제 계약 검사에서 IOS와 ANDROID가 직접 URL을 반환하고 Range 요청까지 통과했다(ADR-032). 앱 화면은 아직 내장 테스트 음원을 재생하며 실제 곡 재생은 KM-057·KM-058에서 연결한다. 실기기 연결 이후의 변경 파일·실행 명령·인수 조건과 제한은 [종합 보고서](docs/DEVICE_RESUME_REPORT.md)에 정리했다.
...
KM-056 검증에서 일반 단위 31개·lint·assembleDebug·assembleRelease와 실제 계약 4개가 모두 통과했다(종료 코드 0). 마지막 실기기 회귀 검사(KM-040)는 Samsung SM-T220의 계측 10개가 통과했다. 이후 소스 POC는 앱 재생 화면에 아직 연결하지 않았다. 린트는 오류 0개·경고 18개이며 경고를 억제하지 않았다. 작업별 추가 검증과 한계는 순차 구현 기록을 참고한다.
...
실제 외부 요청은 일반 단위 검사에서 제외되어 있다. 명시적으로 검사하려면 다음을 실행한다. 스트림 검사는 후보 클라이언트를 모두 시도한 뒤 종류별 결과를 요약 출력하며, 어떤 종류에서도 직접 재생 가능한 오디오 스트림을 얻지 못하면 실패한다. 공급자가 전송 방식을 바꾸면 이 검사가 먼저 깨진다.
