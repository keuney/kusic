...
**현재: 완료 66 / 미착수 11. 보류 1건(KM-135, 조건 미충족). 작업 트리 깨끗하고 브랜치는 main 하나다. KM-132·133·135·136·151·152 커밋은 아직 origin에 올리지 않았다.** 상태 표시를 세어 확인한 값이며 미착수 11에는 최종 게이트 KM-200과 보류한 KM-135가 포함된다.

**M6 Player UX 완료 (KM-090~097). M7 Library 완료 (KM-110~116). M8은 블루투스 두 건(KM-130·131)과 조건 미충족 한 건(KM-135)을 빼고 완료.**

**다음 작업: KM-153 (Settings).** M9의 마지막이며, 재생 화면에 임시로 둔 화면 색(KM-152)과 WiFi 전용 재생(KM-137)을 그리로 옮기는 일이 포함된다. 초기 항목은 theme·cache size·clear cache·history enabled다.

**KM-152에서 정한 것:** 색은 Material 3 기본 배색. 시스템 표시줄 아이콘 색은 시스템 설정이 아니라 앱이 그리는 색을 따른다. 앱 전체 설정은 SettingsViewModel, 재생 관련 설정은 PlayerViewModel이 갖는다(ADR-067).
