...
**블루투스 기기가 생기면** KM-130·131과 KM-200의 나머지 한 항목, 그리고 사용자 키로 서명한 APK 설치(KM-158의 남은 검증)를 함께 확인한다.
...
완료 (2026-09-03): 재생목록 목록은 라이브러리 탭 구획에, 곡 목록은 전체 화면 목적지에, 담기 진입점은 Now Playing에 뒀다. 방향은 코덱스가 정했다.

- 인수 조건: create PASS, rename PASS, delete PASS, add track PASS, remove track PASS. 다섯 개 모두 실기기에서 확인했다.
- feature/library/PlaylistScreen과 PlaylistDialogs를 추가하고 LibraryScreen에 재생목록 구획을 더했다. 이 프로젝트에서 대화상자가 처음 등장한다.
...
- 재생목록 화면의 곡을 누르면 그 곡부터 재생하고 목록 전체가 대기열이 된다. 누르면 아무 일도 없는 줄을 두지 않기 위해 연결했다. 이어 듣기와 이전·다음 확인은 KM-114다.
- 삭제에 확인을 묻지 않는다. 대신 한 번 더 들어가야 닿는 자리에 뒀다. 되돌리기가 없다는 점은 남는 위험이다.
- 단위 6개 추가.
- `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0(5분 7초). 단위 153개·실제 계약 7개·실기기 계측 43개. 린트 오류 0.
- 결정은 ADR-059에 기록했다. 신규 의존성 없음.
...
완료 (2026-09-04): 세 구획은 KM-112·113·115에서 이미 들어왔으므로 실제 사용에서 무너지는 지점을 고쳤다.

- 인수 조건: Sections PASS(Favorites·Playlists·History 세 구획). 순서는 PRD 35대로 최근 재생 → 즐겨찾기 → 재생목록이다.
- 곡 구획은 앞의 다섯 개만 보여주고 나머지는 "더 보기"로 넘긴다. 최근 재생은 50개까지 쌓이고 즐겨찾기는 제한이 없어 전부 쏟으면 재생목록 구획이 화면 밖으로 밀린다.
- 전체 목록은 목적지 library-section/{section} 하나가 구획 이름을 받아 그린다. 알 수 없는 이름이면 최근 재생을 보여준다.
- 즐겨찾기 전체 목록에는 줄마다 해제 버튼을 뒀다. KM-112에서 남겨 둔 "목록에서 바로 해제"가 채워졌다. 요약 화면에는 두지 않는다(훑어보다 실수로 누르기 쉽다).
- 세 구획이 모두 비면 한 줄 안내와 재생목록 만들기 버튼만 보여준다. "없습니다" 세 줄은 무엇을 해야 하는지 알려주지 않는다.
- 재생목록 구획은 전부 보여준다. 줄이 짧고 개수가 적어 아래를 밀어내지 않는다.
- 단위 3개 추가.
- `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0(4분 53초). 단위 161개·실제 계약 7개·실기기 계측 45개. 린트 오류 0.
- 결정은 ADR-062에 기록했다. 신규 의존성 없음. M7 Library 완료.
...
완료 (2026-09-04): 실기기에서 31분 5초 동안 이어서 쓰고 크래시가 없음을 확인했다. 코드 변경 없음.

- 실기기 SM-T220 / Android 14. 로그를 비우고 새로 시작해 10:52:59~11:24:04 동안 사용했다. 그 사이 프로세스는 pid 6716 하나로 다시 시작한 적이 없다.
- search PASS: "jazz", "jazzpiano+sleep" 두 번 검색, 두 번 다 결과가 왔다.
- multiple tracks PASS: 결과 전체를 대기열로 만들고 다음·이전으로 이동, 대기열에서 항목 빼기와 다른 항목 재생, 셔플·반복 토글, 즐겨찾기, 재생목록 만들기.
- background PASS: 홈으로 나간 뒤 6분, 재생이 2.8초→367초로 이어짐.
- screen off PASS: 화면 끈 뒤 7분, Dozing 상태에서 799초까지 이어짐.
- lockscreen PASS: 잠긴 상태에서 미디어 버튼 play/pause와 다음이 동작했다. 잠금화면 조작과 Bluetooth AVRCP가 쓰는 것과 같은 경로다(미디어 버튼 처리 확인이며 Bluetooth 전송 자체는 아니다).
- Bluetooth: 기기가 없어 확인하지 못했다(AGENTS.md 20). KM-130·131과 함께 남는다.
- Crash 없음 PASS: `logcat -b crash`에 이 패키지 기록 없음(버퍼 전체 0줄), main 로그에 FATAL EXCEPTION·ANR in·Force finishing 없음.
- **찾은 것(후속 후보, 이 작업에서 고치지 않음):** 재생할 수 없는 곡에서 재생이 멈춰 선다. 여섯 곡을 시도해 셋이 Source error였고(Playlist형 긴 영상들) 문구와 다시 시도 버튼은 맞게 나오지만 대기열에 다음 곡이 있어도 넘어가지 않는다. 30분 듣는 동안 한 곡만 걸려도 음악이 끊긴다. 자동으로 다음 곡으로 넘기는 처리가 필요하다.
- 그 밖에 찾은 것: 제목이 두 줄인 곡에서 Now Playing의 셔플·반복 칩이 화면 밖에 있어 스크롤해야 닿는다. 즐겨찾기가 다섯 곡 이하면 전체 목록 화면으로 갈 수 없어 해제하려면 그 곡의 Now Playing까지 가야 한다(KM-116 설계의 결과).
- 확인 후 되돌린 것: 셔플·반복(설정 저장값)·재생 상태·최근 재생·즐겨찾기·재생목록·화면 꺼짐 시간·자동 회전. 라이브러리가 빈 상태로 돌아온 것을 확인했다.
- `gradlew.bat test lint assembleDebug assembleRelease sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0. 코드 변경이 없어 실기기 계측은 KM-133의 46개 통과 결과가 유효하다.
- 설계 결정이 없어 ADR을 남기지 않았다.
...
KM-138 — 재생 불가 곡 자동 넘김

Status: [x]

완료 (2026-09-04): 소스 실패에서만 다음 곡으로 넘어가고, 지나간 곡을 화면이 말한다.

- 인수 조건: 소스 실패 자동 넘김 PASS. 네트워크 실패는 넘어가지 않음 PASS. 무한 반복 없음 PASS(넘김 횟수를 대기열 크기로 제한). 지나간 사실이 보임 PASS. 마지막 곡 실패 시 문구·다시 시도 PASS.
- 판단은 UnplayableSkip이 상태만으로 하고 실제로 넘기는 일은 MusicService가 한다. 화면이 닫혀 있어도 음악이 이어져야 한다.
- 설정이 막은 것도 넘기지 않는다. PlaybackFailure.Blocked를 더해 갈랐다(WiFi 전용 재생). 소스 실패로 취급하면 막힌 대기열을 끝까지 훑기만 한다. 덤으로 재생 화면이 실제 이유를 말하게 됐다.
- 지나갔다는 사실은 세션 extras로 보낸다. 컨트롤러가 추론할 수 없다. 오류가 나자마자 서비스가 넘기므로 컨트롤러에는 오류가 도착하기도 전에 다음 곡이 재생된다(계측으로 확인했다). 제목과 횟수를 함께 보내 다시 연결할 때 옛 제목을 지금 일처럼 보여 주지 않는다.
- 안내는 사용자가 무엇을 재생할지 바꿀 때 지운다. 시간으로 지우지 않는다.
- 단위 9개·계측 1개 추가. 계측은 없는 Track ID로 실패를 만들어 넘김·안내·안내 지움을 함께 본다.
- `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0(5분 56초). 단위 192개·실제 계약 7개·실기기 계측 47개. 린트 오류 0.
- 결정은 ADR-072에 기록했다.

Background:

KM-136 실기기 30분 사용 검사에서 시도한 여섯 곡 중 셋이 공급자 쪽 이유로 해석되지 않았다(Source error). 문구와 다시 시도 버튼은 맞게 나오지만 **대기열에 다음 곡이 있어도 넘어가지 않는다.** 한 곡만 걸려도 음악이 끊기고 사용자가 손을 대야 한다. 검색 결과 전체가 대기열이 되는 구조(KM-097)에서는 걸릴 확률이 높다.

Goal:

공급자 쪽 이유로 재생할 수 없는 곡에서 대기열이 멈춰 서지 않게 한다.

Acceptance Criteria:

소스 실패이고 대기열에 다음 곡이 있으면 자동으로 다음 곡으로 넘어간다

네트워크 실패는 넘어가지 않는다(연결이 돌아오면 그 곡을 이어 듣는다, ADR-063)

연달아 실패해도 무한히 돌지 않는다(대기열을 한 번 훑고 멈춘다)

넘어갔다는 사실이 사용자에게 보인다

마지막 곡이 실패하면 지금처럼 문구와 다시 시도를 보여준다

Notes:

실패 종류를 가르는 것은 이미 있다(`PlaybackFailure`, ADR-063). 자동 넘김은 화면이 닫혀 있어도 동작해야 하므로 재생을 소유한 `MusicService`가 판단한다(ADR-002). 넘김 횟수 제한은 `NetworkRecovery`와 같은 방식으로 상태를 떼어 단위 검사로 고정할 수 있다.

KM-139 — 최근 재생 셔플

Status: [x]

완료 (2026-09-08): 최근 재생 목록을 한 번 눌러 무작위 순서로 재생한다. 무작위는 대기열 자체에 넣는다.

- 인수 조건: 한 번 눌러 무작위 순서로 시작 PASS(홈·라이브러리 요약·최근 재생 전체 목록 세 곳). 곡이 빠지거나 겹치지 않음 PASS. 순서가 고정되어 있지 않음 PASS. 대기열 화면의 순서가 실제 재생 순서 PASS. 빈 목록이면 버튼 없음 PASS.
- **셔플 모드는 켜지 않고 끈다.** Media3의 셔플 순서는 세션 뒤에 있어 컨트롤러가 읽을 수 없다(ADR-053). 켜 두면 이미 섞은 대기열을 다시 섞고 대기열 화면이 보여 주는 순서가 실제 순서가 아니게 된다. 무작위는 한 곳에만 둔다.
- 섞는 규칙은 `shuffledQueue`로 떼어 두고 `Random`을 받아 기기 없이 순서를 고정해 검사한다. 받은 목록은 바꾸지 않는다(화면이 관찰 중인 목록이다).
- 실기기(SM-T220 / Android 14, 최근 재생 6곡): 세 곳에서 각각 눌러 재생이 시작됐고 시작한 곡이 목록의 첫 곡이 아니었다. 대기열에 6곡이 그대로 있고 겹치지 않았다. 다음 버튼이 대기열 두 번째 줄로 갔다(보이는 순서가 재생 순서다). 두 번 눌러 서로 다른 순서를 얻었다. 재생 화면의 셔플 표시는 꺼진 상태였다. crash 0건.
- 단위 7개·계측 1개 추가. `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0(7분 24초). 단위 199개·실제 계약 7개·실기기 계측 48개, 실패/오류 0. 린트 오류 0·경고 22.
- 결정은 ADR-073에 기록했다. 신규 의존성 없음.

Background:

사용자 요청(2026-09-08). 최근에 들은 곡들을 무작위 순서로 이어 듣고 싶다.

지금은 셔플이 재생 화면의 토글 하나뿐이다. 최근 재생 목록을 섞어 들으려면 곡 하나를 눌러 대기열을 만들고, 재생 화면으로 들어가 토글을 켜야 한다. 그러고도 이미 시작한 첫 곡은 바뀌지 않는다. 누른 곡부터 재생하는 것은 "이 곡을 듣겠다"는 뜻이고, 목록을 섞어 듣는 것은 "무엇이든 듣겠다"는 뜻이라 시작하는 방법이 따로 있어야 한다.

Goal:

최근 재생 목록이 보이는 자리에서 한 번 눌러 그 목록을 무작위 순서로 재생하기 시작한다.

Acceptance Criteria:

최근 재생이 보이는 화면에서 한 번 눌러 무작위 순서로 재생이 시작된다

목록의 곡이 하나도 빠지지 않고 하나도 겹치지 않는다

순서가 고정되어 있지 않다

대기열 화면에 보이는 순서가 실제로 재생될 순서다

최근 재생이 비어 있으면 누를 것이 없다

Notes:

무작위를 대기열 자체에 넣는다. Media3의 셔플 모드는 재생 순서를 세션 뒤에서 섞고 그 순서를 컨트롤러가 읽을 수 없어(ADR-053) 대기열 화면이 보여 주는 순서와 실제 순서가 어긋난다. 앱이 순서를 정해 넣으면 보이는 것이 그대로 재생된다.

섞는 규칙은 기기 없이 고정할 수 있게 떼어 둔다(`UnplayableSkip`·`RepeatCycle`과 같은 방식).
...
완료 (2026-09-04): 세 구획이 KM-116의 라이브러리 화면과 같은 것이어서, 무엇으로 가를지 정하는 것이 이 작업의 일이었다.

- 인수 조건: Sections PASS(Recently Played·Favorites·Playlists). PRD 35 순서를 따른다.
- 가른 기준: 홈은 한 번 눌러 다시 듣는 자리, 라이브러리는 전부 보고 정리하는 자리. PRD는 홈에 무엇이 있어야 하는지만 정하므로 모양으로 가른다. 홈은 가로로 흐르는 큰 정사각형 카드, 라이브러리는 세로 목록이다.
- 홈에는 정리 버튼(지우기·해제·이름 바꾸기)이 없다. 다시 듣기 위해 들어온 자리에서 실수로 지우는 일이 없어야 한다.
- 홈에서는 비어 있는 구획을 그리지 않는다. 세 구획이 다 비면 한 줄 안내와 "검색으로 가기"만 둔다.
- 카드 줄은 개수를 자르지 않는다. 가로 줄은 보이는 것만 만들고, 자르면 누른 자리와 대기열의 자리가 어긋날 위험만 생긴다.
- 시작 목적지를 검색에서 홈으로 옮겼다. 자리표시자 NotReadyScreen과 screen_not_ready 문자열은 지웠다.
- 새 데이터 경로 없음. 홈은 라이브러리와 같은 LibraryViewModel(Activity 범위)을 본다.
- 새 단위 검사는 없다. 새로 판단하는 규칙이 없고 기존 상태를 다른 모양으로 그리기만 한다. 대신 실기기에서 확인했다.
- 실기기 확인: 새로 시작하면 홈이 먼저 열린다. 빈 홈의 안내와 검색으로 가기 버튼, 한 곡을 듣고 즐겨찾기·재생목록을 만든 뒤의 세 구획 카드, 카드로 재생, 재생목록 카드로 화면 열기, 가로 화면까지 확인했다.
- `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0(5분 22초). 단위 171개·실제 계약 7개·실기기 계측 46개. 린트 오류 0.
- 결정은 ADR-066에 기록했다. 신규 의존성 없음.
...
완료 (2026-09-04): ThemePreference와 DataStore 저장은 이미 있었고 아무도 읽지 않고 있었다. 이 작업이 화면에 연결했다.

- 인수 조건: System PASS, Light PASS, Dark PASS, 주요 화면 대비 PASS(실기기 확인).
- 색은 Material 3 기본 배색을 그대로 쓴다. 고유 브랜드 색을 정한 적이 없고 기본 배색은 대비가 맞춰져 있다.
- 합치는 규칙은 ThemePreference.isDark(systemDark) 한 줄. 시스템은 기기를 따르고 밝게·어둡게는 기기와 달라도 그것을 따른다. 단위 3개로 고정했다.
- 시스템 표시줄 아이콘 색이 앱 색을 따르게 고쳤다. 이전에는 밝은 쪽으로 고정돼 어두운 화면에서 아이콘이 보이지 않았다.
- 시작 창에 밝게·어둡게 두 벌을 뒀다(values/themes.xml, values-night/themes.xml). 어두운 화면으로 들어갈 때의 흰 번쩍임을 없앤다. 이 값은 시스템 설정만 따를 수 있다.
- 고르는 자리는 재생 화면에 임시로 뒀다. WiFi 전용 재생과 함께 KM-153에서 설정 화면으로 옮긴다.
- 앱 전체 설정은 새 SettingsViewModel이 갖는다. KM-153이 나머지를 여기에 더한다.
- 실기기 확인: 시스템 밝게/어둡게 + 시스템 설정, 시스템 어둡게 + 앱 밝게, 시스템 밝게 + 앱 어둡게 네 경우 모두 표시줄과 본문이 읽힌다. 검색 결과·라이브러리·재생 화면·미니 플레이어·하단 탭의 어두운 색 대비도 확인했다.
- `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0(5분 32초). 단위 174개·실제 계약 7개·실기기 계측 46개. 린트 오류 0.
- 결정은 ADR-067에 기록했다. 신규 의존성 없음.
...
완료 (2026-09-04): PRD가 정한 네 가지만 뒀다. 재생 화면에 임시로 두었던 설정 두 개를 여기로 옮겼다.

- 인수 조건: theme PASS, cache size PASS, clear cache PASS, history enabled PASS. 쓰이지 않는 설정은 넣지 않았다.
- 옮긴 것: WiFi 전용 재생(KM-137), 화면 색(KM-152). 반복은 옮기지 않았다. 지금 듣는 것을 바꾸는 조작이고 셔플과 나란히 있어야 뜻이 통한다.
- 설정으로 가는 길은 홈 화면의 버튼 하나다. 하단 탭은 PRD 35가 세 개로 정해 두었다.
- 캐시 상한(128·256·512MB)은 PlaybackCache가 만들 때 읽으므로 다음 실행부터 적용된다. Media3 evictor는 만든 뒤 상한을 바꿀 수 없다. 화면은 지금 걸린 상한으로 사용량을 말하고 고른 값과 다를 때만 안내를 붙인다. PlaybackCache.MAX_BYTES 상수는 CacheLimit로 옮겼다.
- 재생 기록 끄기는 KM-115에서 미뤄 둔 것이다. 끄면 그때부터 남기지 않고 이미 남은 기록은 둔다.
- 실기기에서 결함을 하나 찾아 고쳤다. 예약된 작업 안에서 설정을 물어보면, 껐다 켠 뒤 지금 곡이 영영 기록되지 않는다("이미 처리했다" 표시만 남기 때문). 서비스가 값을 들고 있다가 예약 전에 보도록 바꾸고, 설정이 바뀌면 판단을 한 번 더 돌려 켜는 순간 지금 곡부터 남게 했다.
- 단위 9개 추가(크기 표기 4개, 설정 저장 5개). 검사용 SettingsRepository 대역 세 곳도 함께 고쳤다.
- 실기기 확인: 기록을 끈 채 18초를 들어도 최근 재생이 비어 있고, 켜면 남으며 듣는 중에 켜도 그 곡이 남는다. 상한 512MB를 고르면 안내가 나오고 다시 시작하면 적용된다. 재생 뒤 사용 중 3MB였고 지우니 0KB가 됐다.
- `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0(5분 49초). 단위 183개·실제 계약 7개·실기기 계측 46개. 린트 오류 0.
- 결정은 ADR-068에 기록했다. 신규 의존성 없음.
...
완료 (2026-09-04): 고치기 전에 재어 봤다. 세 조건 중 둘은 이미 충족돼 있었고, 읽는 쪽이 이름을 알 수 없던 곳을 고쳤다.

- 인수 조건: media controls contentDescription PASS(이미 모두 붙어 있었다. 이전·재생·다음은 글자 버튼이라 그 자체가 이름이다). touch target PASS(기기에서 재어 보니 누를 수 있는 모든 것이 47~48dp. Material 3이 최소 터치 영역을 강제한다). large font PASS(font_scale 1.5에서 설정·재생·대기열·홈 모두 잘림 없음).
- 고친 것 1: 커스텀 클릭에 onClickLabel을 붙였다. 곡 줄·홈 카드·대기열 줄은 "재생", 재생목록은 "열기", 미니 플레이어 줄은 "전체 화면 플레이어 열기", 담기 항목은 "재생목록에 담기".
- 고친 것 2: 재생 상태 줄을 live region으로 표시해 재생·일시정지·오류가 바뀔 때 읽어 주게 했다.
- 고친 것 3: 위치 줄을 한 덩어리로 묶어 "재생 위치 0:09 / 2:00"으로 읽히게 했다.
- 못 한 것: 슬라이더 자체에는 이름을 붙이지 못했다. Modifier.semantics로 붙이면 SeekBar 노드와 이름 노드가 따로 생기고(mergeDescendants도 안 됨) 이름 노드는 focusable=false여서 읽히지 않을 수 있다. 확인할 수 없는 것을 남기지 않고 걷어냈으며, 바로 아래 위치 줄이 그 뜻을 말한다(ADR-069).
- 새 단위 검사는 없다. Compose UI 검사 도구가 없어(새 의존성을 들이지 않았다) 기기의 접근성 트리 덤프로 확인했다.
- `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0(5분 44초). 단위 183개·실제 계약 7개·실기기 계측 46개. 린트 오류 0.
- 결정은 ADR-069에 기록했다. 신규 의존성 없음.
...
완료 (2026-09-04): README의 "현재 상태"가 KM-039 시점에 멈춰 있었다. 다시 썼다. 코드 변경 없음.

- 인수 조건: project purpose PASS, personal sideload boundary PASS, build instructions PASS, architecture summary PASS, known limitations PASS.
- 손대기 전에는 "검색은 아직 앱 화면과 연결하지 않았으며 라이브러리 화면도 없다"고 적혀 있었다. 처음 보는 사람에게 거짓을 말하는 문서였다.
- 필수 검증 명령을 지금 쓰는 한 줄로 갈았다. 이전에는 계측·계약 검사가 빠진 네 줄이었고 통과 수치도 KM-071 시점(단위 73개)이었다. 현재 수치(183·7·46)로 고쳤다.
- 아키텍처 요약은 세 겹과 다섯 규칙(재생 소유권·MusicSource 격리·불변 상태·Room/DataStore 분담·캐시 위치)으로 정리하고 ADR을 가리켰다. 기존 "설계 방향" 절은 이것으로 대체해 지웠다.
- 알려진 한계 아홉 가지를 적었다(블루투스 미확인, 재생 불가 곡에서 멈춤, 셔플 순서 표시 불가, 캐시 상한 적용 시점, 슬라이더 접근성 이름, 긴 제목에서 칩 밀림, 어두운 화면 첫 프레임, OEM 절전 미재현, progressive 데이터 사용량). 모두 저장소 기록에서 나온 것이다.
- 디렉터리 구조를 갱신했다. 이전 트리에는 feature/player만 있었다.
- 스트림 URL 미저장 설명에서 검사 코드를 확인하고 artwork_url 예외를 반영했다.
- `gradlew.bat test lint assembleDebug assembleRelease sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0. app/ 변경이 없어 계측은 KM-154의 46개 결과가 유효하다.
- 설계를 정한 것이 없어 ADR을 남기지 않았다.
...
완료 (2026-09-04, 한계 있음): 릴리스 빌드를 실기기에 설치해 확인했다. 배포용 서명은 사용자 키가 없어 확인하지 못했다.

- 인수 조건: release APK installs PASS(단, 디버그 키로 서명한 릴리스 빌드로 확인했다). core smoke test PASS.
- 서명이 아예 없는 APK는 설치되지 않는다(INSTALL_PARSE_FAILED_NO_CERTIFICATES). 사용자가 "키 만들기 없이 진행"을 요청했으므로 그 뜻에 맞는 가장 가까운 방법으로 했다. 디버그 키는 SDK가 만든 공개된 키라 사용자가 만들 것이 없고, 기기에 깔린 앱과 서명이 같아 지우지 않고 덮어 설치된다.
- KM-157의 환경 변수 경로를 써서 저장소에 파일을 하나도 남기지 않았다. 그 경로의 실제 검증도 됐다.
- 설치된 것이 릴리스 변형임을 확인했다. dumpsys package의 flags에 DEBUGGABLE이 없다(디버그 빌드에는 있었다). 서명은 apksigner verify로 CN=Android Debug를 확인했다.
- 스모크 검사(SM-T220 / Android 14, 프로세스 하나로 끝까지): 검색 → 재생 → 홈으로 45초 → 화면 끄고 60초 → 잠긴 화면 미디어 버튼 → 알림·foreground service 유지 → 라이브러리 최근 재생 기록. crash·ANR 없음.
- **남은 검증: 사용자 키로 서명한 APK의 설치.** 사용자가 키를 만들면 keystore.properties를 두고 같은 절차를 다시 한다.
- `gradlew.bat test lint assembleDebug assembleRelease sourceContractTest -PsourceContractUseWindowsTrust=true --continue`: PASS, 종료 코드 0. app/ 변경이 없어 계측은 KM-157의 46개 결과가 유효하다.
- 설계를 정한 것이 없어 ADR을 남기지 않았다.
...
사용자 요청(2026-09-09). KM-158에서 만든 배포용 APK가 `0.1.0` / 코드 `1`이고, 그 뒤 KM-138(재생 불가 곡 자동 넘김)과 KM-139(최근 재생 셔플)가 들어간 APK도 같은 값이다. 기기에 이미 깔린 것과 새로 만든 것을 앱 안에서 구분할 수 없고, `versionCode`가 같으면 나중에 어느 쪽이 새 것인지 파일 이름에만 의존해야 한다.

버전 이름은 사용자가 `0.2.0`으로 정했다. KM-158의 배포 이후 KM-139로 사용자에게 보이는 기능이 늘었으므로 semver의 minor 증가에 해당한다. PRD는 v0.2를 다른 뜻으로 예약해 두지 않았다.

Goal:

배포하는 빌드의 버전을 올리고, 다음에 또 배포할 때 무엇을 해야 하는지 남긴다.

Acceptance Criteria:

versionCode가 이전 배포보다 크다

versionName이 0.2.0이다

이전 버전이 깔린 기기에 덮어 설치되고 로컬 데이터가 남는다

다음 배포에서 버전을 올려야 한다는 것이 문서에 있다
