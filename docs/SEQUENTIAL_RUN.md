...
## KM-200 완료 — Final MVP Gate (항목 하나 미검증)

- 브랜치 `codex/KM-200-final-gate`. 코드 변경 없음. 사용자가 "나머지 15개를 정식 검증하고 Bluetooth만 미검증으로 분리 보고"(1번)를 선택했다.
- **게이트는 릴리스 빌드에서 돌렸다.** KM-158에서 설치한 것(디버그 키 서명, `flags`에 DEBUGGABLE 없음)을 그대로 썼다. 디버그 빌드로 게이트를 통과시키면 사용자가 실제로 쓰는 것과 다른 것을 검증하는 셈이다.
- 로그를 비우고 force-stop 후 새로 시작해 17:01부터 한 프로세스(pid 7208)로 끝까지 진행했다.
- Must PASS 15개 중 **14개 PASS, 1개 미검증**. 판정 표는 TASKS.md의 KM-200 기록에 있다. 근거를 남긴 방식은 다음과 같다.
  - 검색·재생·다음: 실제 검색("guitar") 결과에서 재생하고 다음 버튼으로 `active item id`가 0→1로 바뀌며 제목도 바뀌는 것을 확인했다.
  - 배경·화면 꺼짐: 각각 32초를 두고 위치가 123.9초·158.7초로 나아가는 것을 세션 상태에서 읽었다.
  - **알림 조작은 실제 알림 버튼을 눌렀다.** 알림창을 펼쳐 uiautomator로 버튼 좌표를 찾고 재생·다음 트랙을 눌러 상태가 바뀌는 것을 확인했다. 미디어 버튼(keyevent)으로 대신하지 않았다. 그것은 잠금화면 항목의 근거다.
  - 잠금화면: 잠긴 상태에서 미디어 버튼으로 재생/일시정지와 다음이 동작했다.
  - **지속 세 항목은 force-stop 후 재실행으로 확인했다.** 즐겨찾기 1곡, 재생목록 "gate"(1곡), 최근 재생 4곡이 그대로 있었다. 같은 세션 안에서만 보는 것으로는 "persist"를 판정할 수 없다.
  - 네트워크 오류: WiFi를 끊고 버퍼 밖으로 옮겨 "네트워크가 끊겼습니다. 연결되면 이어서 재생합니다."와 다시 시도 버튼을 확인했다. WiFi를 켜자 **화면을 건드리지 않고** 재생이 재개됐다. 이때 옮긴 자리가 곡 끝 근처(261초/274초)여서 남은 구간을 재생한 뒤 다음 곡으로 넘어갔다. 같은 곡 같은 자리에서 이어지는 모습은 KM-132 기록에 있다.
  - 크래시: 게이트 전 구간에서 `logcat -b crash`에 이 패키지 기록이 없고 FATAL·ANR이 0건이다. ExoPlayer 오류 1건은 네트워크 검사에서 의도한 것이며 앱이 문구와 자동 회복으로 처리했다.
- **미검증: Bluetooth Play/Pause.** 블루투스 기기가 없다. 같은 경로인 미디어 버튼 처리는 확인했지만 그것을 "Bluetooth 통과"로 적지 않는다(AGENTS.md 20).
- 필수 검증: `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue` PASS, 종료 코드 0(5분 3초). 단위 183개·실제 계약 7개·실기기 계측 46개, 실패/오류 0. 린트 오류 0·경고 22.
- 확인 후 gate 재생목록·즐겨찾기·최근 재생을 지우고 재생을 멈췄으며 화면 꺼짐 시간을 30000으로 되돌렸다. 기기에는 릴리스 빌드를 다시 설치해 두었다(환경 변수로 디버그 키를 써서 저장소에 남는 파일은 없다).
- 화면 캡처는 captures/km-200에 보관했다(저장소 추적 대상 아님).
- ADR을 남기지 않았다. 설계를 정한 것이 없고 확인만 했다.
- **남은 검증 세 가지:** Bluetooth Play/Pause(KM-130), 헤드셋 분리(KM-131), 사용자 키로 서명한 APK 설치(KM-158). 모두 하드웨어나 사용자 키가 필요하며 코드 문제가 아니다.

## 백로그 추가 — KM-138 재생 불가 곡 자동 넘김 (2026-09-04)

- 사용자 요청으로 KM-136에서 찾은 문제를 작업으로 만들었다. M8에 KM-138로 넣었다. KM-137을 새로 추가했던 것과 같은 방식이다.
- 왜 작업으로 만드는지: 공급자 쪽 이유로 해석되지 않는 곡이 적지 않고(30분 검사에서 여섯 곡 중 셋), 그 곡에서 대기열이 멈춰 선다. 검색 결과 전체가 대기열이 되는 구조에서는 걸릴 확률이 높다. v0.1 인수 조건에는 없지만 실사용에서 가장 크게 걸리는 부분이다.
- 인수 조건에 **넘어가지 말아야 하는 경우**를 함께 넣었다. 네트워크 실패는 넘기지 않는다. 연결이 돌아오면 그 곡을 이어 들어야 한다(ADR-063). 실패 종류를 가르는 것은 이미 `PlaybackFailure`에 있다.
- 무한 반복 방지도 인수 조건이다. 대기열의 모든 곡이 실패하면 한 번 훑고 멈춰야 한다.
- 같은 날 KM-132~200의 13개 커밋을 사용자 요청으로 origin에 push했다. push 전에 추적 파일에서 서명 비밀·키스토어 흔적이 없음을 확인했다(`git grep`으로 비밀번호 패턴, `git log --name-only`로 키 파일 이름).

## KM-138 완료 — 재생 불가 곡 자동 넘김

- 브랜치 `codex/KM-138-skip-unplayable`. KM-136에서 찾아 사용자 요청으로 만든 작업이다.
- 넘어가는 경우는 하나뿐이다. 곡 자체를 가져올 수 없고(`PlaybackFailure.Source`), 듣던 중이었고, 대기열에 다음 곡이 있을 때. 판단은 `UnplayableSkip`이 상태만으로 하고 실제로 넘기는 일은 `MusicService`가 한다. 화면이 닫혀 있어도 음악이 이어져야 한다.
- 네트워크 실패는 넘기지 않는다(연결이 돌아오면 그 곡을 이어 듣는다, ADR-063).
- **설정이 막은 것도 넘기지 않는다.** 이를 위해 `PlaybackFailure.Blocked`를 더했다. WiFi 전용 재생이 막은 것을 소스 실패로 취급하면 막힌 대기열을 끝까지 훑기만 하고 사용자는 왜 아무것도 재생되지 않는지 알 수 없다. `MeteredNetworkBlockedException`이 전용 오류 코드를 실어 보내 갈라진다. 덤으로 재생 화면이 "이 곡을 재생할 수 없습니다" 대신 실제 이유를 말하게 됐다.
- 넘김 횟수는 대기열 크기로 제한한다. 전체 반복이면 "다음 곡이 있다"가 늘 참이라 모든 곡이 실패할 때 끝없이 돌 수 있다. 한 번이라도 재생되면 횟수는 되돌아간다.
- **시행착오(설계를 바꾼 것).** 처음에는 지나갔다는 사실을 컨트롤러가 스스로 알아내게 했다. "소스 오류가 있었고 그 다음에 다른 곡으로 바뀌었다면 지나간 것"이라는 추론이다. 계측을 돌리니 넘김은 되는데 안내가 비어 있었다(`expected:<없는 곡> but was:<null>`).
- 원인: 오류가 나자마자 서비스가 다음 곡으로 넘기고 다시 준비하므로 **컨트롤러에는 오류 상태가 도착하기도 전에 이미 다음 곡이 재생되고 있다.** 추론으로 알아낼 수 있는 정보가 아니었다. 세션 extras로 제목과 횟수를 직접 보내도록 바꿨다. 횟수를 함께 보내는 이유는 다시 연결할 때 옛 제목을 지금 일어난 일처럼 보여 주지 않기 위해서다.
- 안내는 사용자가 무엇을 재생할지 바꿀 때 지운다. 시간이 지나면 사라지게 하지 않는다. 화면을 보고 있지 않은 사이에 사라지면 무엇이 빠졌는지 영영 알 수 없다.
- 단위 9개 추가: 넘김 규칙 8개(`UnplayableSkipTest`), 막힌 실패 분류 1개(`PlaybackFailureTest`). 183개 → 192개.
- 계측 1개 추가(`SkipUnplayableTest`). **없는 Track ID**를 첫 곡으로 넣어 실패를 만든다. 공급자가 거부하므로 결과가 늘 같고, 네트워크를 끊는 것과 달리 다른 검사에 영향을 주지 않는다. 넘어가 다음 곡이 재생되는 것, 지나간 제목이 상태에 남는 것, 조작하면 안내가 지워지는 것을 함께 본다. 계측 46개 → 47개.
...
- 브랜치 `codex/KM-139-recent-shuffle`. 사용자 요청으로 추가한 작업이다.
- **무작위를 대기열 자체에 넣었다.** Media3의 셔플 모드를 켜는 방법은 쓰지 않았다. 그 순서는 세션 뒤에 있어 컨트롤러가 읽을 수 없고(ADR-053), 그래서 대기열 화면은 "실제 순서는 다르다"는 안내를 달아야 한다. 앱이 순서를 정해 넣으면 보이는 것이 그대로 재생된다.
- 그러므로 셔플 재생은 셔플 모드를 **끈다**. 켜 두면 이미 섞은 대기열을 다시 섞어 화면이 거짓을 보인다. 재생 화면의 토글은 그대로 남는다. 토글은 지금 듣고 있는 대기열의 다음 곡 순서를 바꾸고, 셔플 재생은 무엇을 들을지 새로 정한다.
- 버튼은 최근 재생이 보이는 세 곳(홈·라이브러리 요약·최근 재생 전체 목록)의 구획 머리에 뒀다. 세 곳 모두 화면이 보고 있는 목록 전체를 넘긴다. 요약 화면에 다섯 곡만 보여도 대기열에는 최근 재생 전체가 들어간다.
- 단위 7개 추가(`ShuffledQueueTest`): 곡이 빠지거나 겹치지 않음, 넣은 순서가 아님, 다른 씨앗이면 다른 순서, 같은 씨앗이면 같은 순서, 받은 목록이 바뀌지 않음, 빈 목록, 한 곡. 192개 → 199개.
- 계측 1개 추가(`ShufflePlayTest`). 셔플 모드를 **켠 상태에서 출발**해 셔플 재생 뒤에 꺼지는지 본다. 반대 상태에서 시작하지 않으면 그 규칙이 실제로 동작하는지 알 수 없다. 대기열이 목록 전체로 채워지는지, 첫 곡이 대기열 첫 줄인지, 빈 목록이 대기열을 지우지 않는지도 함께 본다. 계측 47개 → 48개.
- 셔플이 꺼져 있으면 넘어가는 순서가 대기열 목록의 순서라는 것은 ShuffleTest가 이미 순회로 확인했으므로 되풀이하지 않았다. 순서가 매번 다른지는 단위 검사가 씨앗을 고정해 판정한다. 여섯 곡을 실기기에서 몇 번 눌러 비교하는 방식은 우연히 같을 수 있어 판정이 흔들린다.
- `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue --no-daemon --console=plain`: PASS, 종료 코드 0(7분 24초). 단위 199개·실제 계약 7개·실기기 계측 48개, 실패/오류 0. 린트 오류 0·경고 22.
- 실기기 확인(SM-T220 / Android 14, 디버그 빌드): 최근 재생 6곡을 만든 뒤 홈·라이브러리 요약·최근 재생 전체 목록에서 각각 셔플 재생을 눌렀다. 세 번 모두 재생이 시작됐고 시작한 곡이 목록의 첫 곡이 아니었다(Disney → Dance Practice → America's Got Talent). 대기열에 6곡이 그대로 있고 겹치지 않았다. 다음 버튼이 대기열의 두 번째 줄로 갔다. 두 번 눌러 서로 다른 순서를 얻었다. 재생 화면의 셔플 표시는 꺼진 상태였다. crash 버퍼 0줄. 화면 캡처는 captures/km-139에 보관했다(저장소 추적 대상 아님).
- **시행착오(검증 준비).** 최근 재생을 만들려고 "piano" 검색 결과를 눌렀는데 상위 결과가 10시간짜리 편집 영상이라 10초 재생 기준(KM-115)에 닿지 못했다. progressive 형식은 영상까지 받으므로(ADR-034) 긴 영상은 시작이 느리다. 3~4분 곡으로 검색어를 바꿔(BTS Dynamite) 곡마다 20초씩 재생해 6곡을 만들었다.
- **시행착오 2(내 확인 방법의 잘못).** `dumpsys media_session`에서 첫 번째 `state=PlaybackState` 줄을 읽었는데 그것이 지난 세션 항목이라 계속 `BUFFERING position=0`으로 보였고 재생이 안 되는 것으로 오해했다. 실제로는 재생 중이었다. 앱 로그의 마지막 상태로 바꿔 확인했다. 앱의 결함이 아니었다.
- 결정 ADR-073. 신규 의존성 없음.

## 백로그 추가 — KM-159 배포 빌드 버전 올리기 (2026-09-09)

- 사용자 요청. M9에 KM-159로 넣었다. KM-158(릴리스 APK) 다음 자리다.
- 왜: KM-158에서 만든 배포용 APK가 `0.1.0` / 코드 `1`이고, KM-138·KM-139가 들어간 APK도 같은 값이다. 기기에 깔린 것과 새로 만든 것을 앱 안에서 구분할 수 없다. `versionCode`가 같으면 어느 쪽이 새 것인지 파일 이름에만 의존해야 한다.
- 버전 이름을 무엇으로 할지 사용자에게 물었고 `0.2.0`을 받았다. 처음에 사용자가 제안한 값은 `0.1.1`이었고, 나는 "MVP 게이트(KM-200)에 미검증 항목이 남아 있으니 0.1 계열에 머무는 편이 정직하다"는 이유로 그것을 받아들이려 했다. 사용자가 `0.2.0`으로 정했다. KM-158 배포 이후 사용자에게 보이는 기능(KM-139)이 늘었으므로 semver의 minor 증가가 맞고, PRD는 v0.2를 다른 뜻으로 예약해 두지 않았다.
- 인수 조건에 **이전 버전이 깔린 기기에 덮어 설치되고 로컬 데이터가 남는지**를 넣었다. 버전을 올리는 목적이 갱신 설치이므로 그것이 실제로 되는지 봐야 한다. 즐겨찾기·재생목록·기록이 사라지면 올린 의미가 없다.
- 다음 배포에서도 올려야 한다는 것을 문서에 남기는 것도 인수 조건이다. 이번 한 번 올리고 끝나면 같은 일이 반복된다.

## KM-159 완료 — 배포 빌드 버전 올리기

- 브랜치 `codex/KM-159-version-bump`. `versionCode` 1 → 2, `versionName` 0.1.0 → 0.2.0.
- 규칙을 두 곳에 적었다. README의 "배포 빌드 버전" 절과 `defaultConfig`의 주석이다. 버전을 바꾸는 사람이 보는 자리가 그 두 곳이라 한쪽만으로는 다음에 또 잊는다.
- 검사는 추가하지 않았다. 버전은 빌드 설정이고 앱 코드가 그것으로 분기하는 곳이 없다. 확인할 것은 산출물의 값과 실제 갱신 설치다.
- `gradlew.bat test lint assembleDebug assembleRelease connectedDebugAndroidTest sourceContractTest -PsourceContractUseWindowsTrust=true --continue --no-daemon --console=plain`: PASS, 종료 코드 0(6분 30초). 단위 199개·실제 계약 7개·실기기 계측 48개, 실패/오류 0. 린트 오류 0·경고 22.
- 산출물: 환경 변수로 SDK의 공개 debug 키를 지정해 서명한 `app-release.apk`(17,256,377바이트). `aapt2 dump badging`이 versionCode='2' versionName='0.2.0'을 보이고 `apksigner verify`가 CN=Android Debug를 출력한다. 배포용 키는 아니다(KM-158과 같은 한계).
- **갱신 설치 확인(SM-T220 / Android 14).** 새 버전만 설치해 보는 것은 "덮어 설치된다"를 확인한 것이 아니므로, 먼저 0.1.0 APK를 다시 설치해 이전 상태를 만들었다(versionCode=1). 그 상태에서 재생목록 `km159-upgrade-check` 하나와 최근 재생 한 곡(테스트 오디오)을 넣었다. 그 위에 0.2.0을 `install -r`로 얹었더니 Success, versionCode=2 / versionName=0.2.0이고 **재생목록과 최근 재생이 그대로 있었다.** crash 버퍼 0줄.
- 덤으로 확인된 것: 최근 재생이 비어 있는 동안에는 셔플 재생 버튼이 그려지지 않았고, 한 곡이 쌓이자 나타났다(KM-139의 인수 조건 중 하나를 릴리스 빌드에서 다시 본 셈이다).
- 확인 후 검사용 재생목록과 재생 기록을 지웠다. 기기에는 0.2.0 릴리스 빌드가 설치돼 있고 화면 꺼짐 시간은 30000으로 되돌렸다.
- **직전에 있던 일:** 같은 세션에서 KM-139의 커밋 2개를 사용자 요청으로 push했고(`7bbd375..6cb6d59`), 핸드폰 설치용으로 0.1.0 APK를 만들어 `dist/`에 뒀다. 그 APK가 KM-158의 것과 같은 버전이라는 것을 알리면서 이 작업이 나왔다. `dist/keuney-music-0.1.0-km139.apk`는 0.2.0으로 대체해 지웠다(`dist/`의 APK는 `.gitignore` 대상이다).
- 결정 ADR-074. 신규 의존성 없음.
