...
### ADR-074 — 배포 빌드 버전 (KM-159)

- **기기에 넣을 APK를 만들 때마다 `versionCode`를 올린다.** 같은 값이면 기기는 그것이 새 빌드인지 알 수 없다. `versionName`도 함께 바꾼다. 같으면 앱 안에서 무엇이 깔려 있는지 구분할 수 없다. KM-158의 배포 이후 KM-138·KM-139가 들어간 APK가 계속 `0.1.0` / 코드 `1`이었고, 어느 파일이 새 것인지 파일 이름에만 의존해야 했다.
- 이번 값은 `2` / `0.2.0`이다. **사용자가 정했다.** semver로는 KM-158 배포 이후 사용자에게 보이는 기능(최근 재생 셔플)이 늘었으므로 minor 증가가 맞다. PRD는 v0.2를 다른 뜻으로 예약해 두지 않았다. 처음에는 `0.1.1`이 후보였다. MVP 게이트(KM-200)에 미검증 항목이 남아 있어 0.1 계열에 머무는 편이 상태를 정직하게 말한다는 이유였고, 사용자가 기능이 늘었다는 쪽을 택했다.
- **버전으로 앱 동작을 분기하지 않는다.** 그래서 이 변경에는 검사를 추가하지 않았다. 확인할 것은 산출물의 값(`aapt2 dump badging`)과 실제 갱신 설치이며 둘 다 실기기에서 했다.
- 갱신 설치 확인은 **이전 버전을 다시 설치해 이전 상태를 만든 뒤** 그 위에 새 버전을 얹는 방식으로 했다. 새 버전만 설치해 보면 "덮어 설치된다"를 확인한 것이 아니다. 재생목록과 재생 기록이 그대로 남는 것을 함께 봤다. 버전을 올리는 목적이 갱신이므로 데이터가 사라지면 올린 의미가 없다.
- 규칙을 두 곳에 적었다. README의 "배포 빌드 버전" 절과 `defaultConfig`의 주석이다. 버전을 바꾸는 사람이 보는 자리가 그 두 곳이다.
- 서명이 같아야 갱신 설치가 된다는 것은 ADR-071에 이미 적혀 있다. 이번 확인은 그 전제 아래의 결과다. 키가 바뀌면 지우고 다시 설치해야 하며 그때 기기 안의 데이터가 사라진다.
- 근거: [versionCode와 versionName](https://developer.android.com/studio/publish/versioning), [semver 0.y.z](https://semver.org/lang/ko/#spec-item-4).

### ADR-073 — 최근 재생 셔플 재생 (KM-139)

- **무작위를 대기열 자체에 넣는다.** Media3의 셔플 모드로도 순서를 섞을 수 있지만 그 순서는 세션 뒤에 있어 컨트롤러가 읽을 수 없다(ADR-053). 그래서 대기열 화면은 넣은 순서를 보여 주고 "실제 순서는 다르다"는 안내를 달아야 한다(KM-097). 앱이 순서를 정해 넣으면 보이는 것이 그대로 재생되고 그 안내가 필요 없다.
- **그러므로 셔플 재생은 셔플 모드를 켜지 않고 끈다.** 켜 두면 이미 섞은 대기열을 세션이 다시 섞어 화면이 보여 주는 순서가 거짓이 된다. 무작위는 한 곳에만 둔다. 재생 화면의 셔플 토글은 그대로 남는다. 그것은 **지금 듣고 있는 대기열**의 다음 곡 순서를 바꾸는 조작이고, 셔플 재생은 **무엇을 들을지** 새로 정하는 조작이다.
- 섞는 규칙을 `shuffledQueue`로 떼어 두고 `Random`을 인자로 받는다. 씨앗을 주면 순서가 고정되므로 곡이 빠지지 않는지·겹치지 않는지·매번 다른지를 기기 없이 단위 검사로 판정할 수 있다(`UnplayableSkip`·`RepeatCycle`과 같은 방식).
- **받은 목록을 뒤섞지 않고 새 목록을 만든다.** 화면이 관찰 중인 목록을 그대로 넘기므로 제자리에서 섞으면 목록의 줄 순서가 함께 바뀐다.
- 빈 목록이면 아무 일도 하지 않는다. 듣고 있는 것을 빈 대기열로 밀어내지 않는다. 화면에서도 최근 재생이 비어 있으면 버튼을 그리지 않는다.
- 버튼은 최근 재생이 보이는 세 곳(홈·라이브러리 요약·최근 재생 전체 목록)의 **구획 머리**에 둔다. 줄마다 붙는 버튼이 아니라 목록 전체에 대한 조작이므로 지우기와 같은 자리다. 즐겨찾기 구획에는 두지 않았다. 사용자가 요청한 것은 최근 재생이며 쓰이지 않을 버튼을 미리 만들지 않는다.
- 세 곳 모두 **화면이 보고 있는 목록 그대로**를 넘긴다. 요약 화면에 다섯 곡만 보이지만 대기열에는 최근 재생 전체(최대 50곡)가 들어간다. 보이는 다섯 곡만 재생하면 "다시 듣기"가 아니다.
- 검증은 실기기에서 세 곳을 각각 눌러 확인했다. 시작한 곡이 목록의 첫 곡이 아니었고, 대기열에 목록의 곡이 그대로 있었고, 다음 버튼이 대기열의 두 번째 줄로 갔다. 두 번 눌러 서로 다른 순서를 얻었다.
- 근거: [Player.setMediaItems](https://developer.android.com/reference/androidx/media3/common/Player#setMediaItems(java.util.List%3Candroidx.media3.common.MediaItem%3E,int,long)), [kotlin.collections.shuffled](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin.collections/shuffled.html).

### ADR-072 — 재생 불가 곡 자동 넘김 (KM-138)

- **넘어가는 경우는 하나뿐이다.** 곡 자체를 가져올 수 없고(`PlaybackFailure.Source`), 듣던 중이었고, 대기열에 다음 곡이 있을 때. 판단은 `UnplayableSkip`이 상태만으로 하고 실제로 넘기는 일은 재생을 소유한 `MusicService`가 한다(ADR-002). 화면이 닫혀 있어도 음악이 이어져야 한다.
- **네트워크 실패는 넘기지 않는다.** 연결이 돌아오면 그 곡을 이어 들어야 한다(ADR-063). 넘기면 듣던 곡을 잃는다.
- **설정이 막은 것도 넘기지 않는다.** 이를 위해 `PlaybackFailure`에 `Blocked`를 더했다. WiFi 전용 재생이 막은 것은 곡의 문제가 아니므로, 소스 실패로 취급하면 막힌 대기열을 끝까지 훑기만 하고 사용자는 왜 아무것도 재생되지 않는지 알 수 없다. `MeteredNetworkBlockedException`이 전용 오류 코드(`ERROR_CODE_IO_NO_PERMISSION`)를 실어 보내 갈라진다. 덤으로 재생 화면이 "이 곡을 재생할 수 없습니다" 대신 실제 이유를 말하게 됐다.
- 넘김 횟수를 대기열 크기로 제한한다. 반복이 전체 반복이면 마지막 곡 뒤에 처음으로 돌아오므로 "다음 곡이 있다"가 늘 참이 되고, 모든 곡이 재생 불가일 때 끝없이 돌 수 있다. 그것을 막는 것이 이 횟수의 유일한 목적이다. 한 번이라도 재생되면 횟수는 처음으로 돌아간다.
- 마지막 곡이 실패하면 넘어가지 않는다. 그때는 지금까지처럼 문구와 다시 시도가 남는다.
- **지나갔다는 사실은 세션 extras로 화면에 보낸다.** 처음에는 컨트롤러가 스스로 알아내게 했다("소스 오류가 있었고 그 다음에 다른 곡으로 바뀌었다면 지나간 것"). 계측에서 안내가 비어 있어 알게 됐다. 오류가 나자마자 서비스가 넘기고 다시 준비하므로 **컨트롤러에는 오류 상태가 도착하기도 전에 이미 다음 곡이 재생되고 있다.** 추측으로 알아낼 수 있는 정보가 아니었다.
- extras에는 제목과 **횟수**를 함께 보낸다. 제목만 보내면 다시 연결할 때 옛 제목을 지금 일어난 일처럼 보여 준다. 컨트롤러는 붙을 때의 값을 기준으로만 삼고 안내하지 않는다.
- 안내는 **사용자가 무엇을 재생할지 바꿀 때** 지운다. 시간이 지나면 사라지게 하지 않는다. 화면을 보고 있지 않은 사이에 사라지면 무엇이 빠졌는지 영영 알 수 없다. 일시정지나 위치 이동으로는 지우지 않는다. 그것은 지금 곡에 대한 조작이다.
- 계측은 **없는 Track ID**를 첫 곡으로 넣어 실패를 만든다. 공급자가 거부하므로 결과가 늘 같고, 네트워크를 끊는 것과 달리 다른 검사에 영향을 주지 않는다.
- 근거: [Player.seekToNextMediaItem](https://developer.android.com/reference/androidx/media3/common/Player#seekToNextMediaItem()), [MediaSession.setSessionExtras](https://developer.android.com/reference/androidx/media3/session/MediaSession#setSessionExtras(android.os.Bundle)).
...
- 결과: Apache-2.0 236개, BSD-3-Clause 1개(`datastore-preferences-external-protobuf`), MIT 1개(`slf4j-api`). **GPL·LGPL·AGPL은 없다.** 인수 조건이 요구하는 "명시적 문서화"를 이 근거와 함께 적는다.
- `slf4j-api`의 POM에는 라이선스 항목이 없어 같은 버전 `slf4j-bom`에서 MIT를 확인했다. 그 사실도 문서에 적었다. 어디서 알았는지 적지 않으면 다음 사람이 다시 확인할 수 없다.
- **빌드에만 쓰이는 것은 적지 않는다.** KSP 처리기, 검사 라이브러리, Gradle 플러그인은 배포물에 들어가지 않는다. 무엇을 왜 뺐는지 문서에 적었다.
- **라이선스 원문을 배포물에 넣지 않는다.** 이 앱은 개인용 sideload이며 제3자에게 재배포하지 않는다(PRD 1·9). 그래서 종류와 원문 위치만 적는다. 재배포하게 되면 Apache-2.0 4(a)·4(d)의 사본·NOTICE 포함 의무와 BSD·MIT의 표시 의무가 생기며 이 문서만으로 부족하다는 것을 문서 안에 적어 두었다. 조건이 바뀔 때 무엇을 해야 하는지 아는 것이 지금 원문을 붙여 두는 것보다 낫다.
- 버전만 정하는 `*-bom` 세 개는 코드가 들어가지 않지만 목록에 남겼다. 의존성 구성을 그대로 적고 무엇이 플랫폼인지 표시하는 편이, 목록과 실제 구성이 어긋나는 것보다 낫다.
- 이 확인은 시점의 것이다. 의존성을 더하거나 올릴 때 다시 확인하라고 문서에 적었다.
- 근거: [Apache License 2.0 4(a)·4(d)](https://www.apache.org/licenses/LICENSE-2.0#redistribution).

### ADR-069 — 접근성 (KM-154)

- **먼저 재어 보고 필요한 것만 고쳤다.** 아이콘 버튼의 이름은 이미 모두 붙어 있었고(장식용 이미지는 `contentDescription = null`), 터치 영역도 기기에서 재어 보니 누를 수 있는 모든 것이 47~48dp였다. Material 3 구성 요소가 보이는 크기(칩 32dp)와 별개로 최소 터치 영역을 강제하기 때문이다. 큰 글꼴(1.5배)에서도 잘리는 곳이 없었다.
- 그래서 이 작업의 내용은 **읽는 쪽이 이름을 알 수 없던 세 곳**이다.
- **커스텀 클릭에 이름을 붙였다**(`onClickLabel`). 곡 줄·홈 카드·대기열 줄은 "재생", 재생목록은 "열기", 미니 플레이어 줄은 "전체 화면 플레이어 열기", 재생목록 담기 항목은 "재생목록에 담기"다. 없으면 "두 번 눌러 실행"으로만 읽혀 무엇이 일어날지 알 수 없다. Material 3 버튼은 글자가 곧 이름이라 손대지 않았다.
- **재생 상태 줄을 live region으로 표시했다.** 재생·일시정지·오류가 그 한 줄로 바뀐다. 화면을 보지 않는 사람에게도 바뀐 사실이 전해져야 한다.
- **위치 줄에 이름을 붙였다.** "재생 위치 0:09 / 2:00" 한 덩어리로 읽힌다. 두 숫자를 따로 읽으면 무엇의 숫자인지 알 수 없다.
- **슬라이더 자체에는 이름을 붙이지 못했다.** `Modifier.semantics`로 붙이면 기기의 접근성 트리에 슬라이더(SeekBar) 노드와 이름만 가진 노드가 **따로** 생긴다. `mergeDescendants = true`로도 합쳐지지 않았다. 이름 노드는 focusable=false여서 읽히지 않을 수 있다. 확인할 수 없는 것을 고쳤다고 남겨 두지 않기로 하고 걷어냈다. 대신 바로 아래 위치 줄이 그 뜻을 말한다. 슬라이더는 기본 안내(백분율)로 읽힌다.
- 큰 글꼴에서 재생 화면의 셔플·반복 칩이 화면 밖으로 밀리는 것은 그대로 둔다. 세로로 흘리게 해 둔 화면이고(KM-091) 필수 조작(이전·재생·다음)은 1.5배에서도 보인다. 이것은 KM-136에서 이미 적어 둔 관찰이다.
- 근거: [Compose 접근성](https://developer.android.com/develop/ui/compose/accessibility), [최소 터치 영역](https://m3.material.io/foundations/designing/structure#dab862b1-e042-4c40-b680-b484b9f077f6).

### ADR-068 — 설정 화면 (KM-153)

- PRD가 정한 네 가지만 둔다: 화면 색, 캐시 상한, 캐시 지우기, 재생 기록. **쓰이지 않는 설정은 두지 않는다**(작업 지시). 네 가지 모두 동작에 닿는다.
- 여기에 옮긴 것: WiFi 전용 재생(KM-137에서 재생 화면에 임시로 두었던 것)과 화면 색(KM-152). **반복은 옮기지 않았다.** 반복은 지금 듣고 있는 것을 바꾸는 조작이고 재생 화면에서 결과가 곧바로 보인다. 셔플과 나란히 있어야 뜻이 통한다.
- 설정으로 가는 길은 홈 화면의 버튼 하나다. 하단 탭은 PRD 35가 세 개로 정해 두었으므로 탭을 늘리지 않는다. 홈이 시작 화면이라 어디에서든 한 번 눌러 올 수 있다.
- **캐시 상한은 고른 값이 다음 실행부터 적용된다.** Media3의 evictor는 만든 뒤에 상한을 바꿀 방법이 없고, 한 디렉터리의 `SimpleCache`는 프로세스에 하나뿐이며 이미 재생에 쓰이고 있다. 그래서 화면은 **지금 걸린 상한**으로 사용량을 말하고, 고른 값과 다를 때만 "다시 시작할 때 적용됩니다"를 붙인다. 늘 붙어 있으면 읽지 않게 된다.
- 상한을 세 가지(128·256·512MB)로만 준다. 자유 입력은 잘못 넣을 자리를 만들고, 개인용 앱에서 그만큼의 자유가 필요하지 않다.
- 캐시 사용량은 흐름이 아니라 물어보는 값이다. Media3 캐시는 크기가 바뀌었다고 알려 주지 않고, 재생 중에는 계속 늘어나므로 지켜보면 화면이 쉬지 않고 다시 그려진다. 화면에 들어올 때와 비운 뒤에만 읽는다.
- **재생 기록을 끄면 그때부터 남기지 않는다. 이미 남은 기록은 지우지 않는다.** 설정을 끄는 것과 기록을 버리는 것은 다른 뜻이다. 버리는 것은 라이브러리의 지우기가 한다.
- 기록 설정은 서비스가 **값을 받아 들고 있다가** 예약 전에 본다. 예약된 작업 안에서 물어보면, 껐다 켠 사이에 "이미 처리했다"는 표시만 남아 지금 곡이 영영 기록되지 않는다(실기기에서 실제로 그랬다). 켜지는 순간 지금 곡부터 다시 판단하도록 설정이 바뀔 때 판단을 한 번 더 돌린다.
- 크기 표기는 1024 단위로 센다. 상한을 그렇게 정했으므로 다른 기준으로 세면 고른 값과 보이는 값이 어긋난다. MB부터는 소수점을 버린다. 얼마나 찼는지 보려는 값이고 소수점이 판단을 바꾸지 않는다.
- 근거: PRD 34·35, [SimpleCache와 evictor](https://developer.android.com/reference/androidx/media3/datasource/cache/SimpleCache).

### ADR-067 — 어두운 화면 (KM-152)

- **Material 3 기본 배색을 그대로 쓴다.** 이 앱은 고유한 브랜드 색을 정한 적이 없고 PRD도 색을 정하지 않는다. 기본 배색은 밝을 때와 어두울 때의 대비가 이미 맞춰져 있다. 색을 직접 고르면 그 대비를 우리가 지켜야 하고, 지금은 지킬 근거가 없다.
- 저장된 설정과 기기 설정을 합치는 규칙은 `ThemePreference.isDark(systemDark)` 하나다. 시스템은 기기를 따르고, 밝게·어둡게는 기기 설정과 달라도 그것을 따른다. 고르지 않은 사람에게는 기기 설정이 이미 답이고 고른 사람에게는 그 선택이 답이다. 규칙이 한 줄이라 단위 검사로 고정했다.
- **시스템 표시줄의 아이콘 색은 앱이 지금 그리는 색을 따른다.** 시스템 설정이 아니다. 사용자가 시스템은 밝게 두고 앱만 어둡게 고를 수 있기 때문이다. 화면 색이 바뀔 때마다 `enableEdgeToEdge`를 다시 걸어 준다. 이전에는 밝은 쪽으로 고정돼 있어 어두운 화면에서 아이콘이 보이지 않았다.
- `WindowInsetsControllerCompat`를 쓰지 않았다. 그것은 androidx.core에 있고 이 모듈이 선언하지 않은 의존성이다. 이미 선언한 androidx.activity의 `enableEdgeToEdge`로 같은 일을 한다(KM-132에서 선언하지 않은 권한에 기대고 있던 것을 고친 것과 같은 이유다).
- **시작 창에 밝게·어둡게 두 벌을 둔다**(`values/themes.xml`, `values-night/themes.xml`). 앱이 그리기 전의 창 색이라 여기서 정하지 않으면 어두운 화면으로 들어갈 때 흰 창이 한 번 번쩍인다. 이 값은 시스템 설정만 따를 수 있다. 앱 안에서 고른 값은 우리 코드가 돌기 전에 알 수 없어서, 시스템은 밝고 앱만 어둡게 고른 경우에는 시작할 때 한 번 밝게 지나간다.
- 값이 저장소에서 오기 전의 첫 프레임은 기본값(시스템)으로 그린다. 없애려면 값이 올 때까지 화면을 그리지 않아야 하는데 시작이 늦어진다. 개인용 앱에서 아주 짧은 한 번의 어긋남과 시작 지연을 맞바꾸지 않는다.
- 고르는 자리는 임시로 재생 화면에 둔다. 설정 화면(KM-153)이 생기면 WiFi 전용 재생과 함께 그쪽으로 옮긴다. 지금 여기 두는 것은 고른 값이 실제로 적용되는지 볼 자리가 아직 없기 때문이다.
- 앱 전체에 걸리는 설정은 `SettingsViewModel`이 갖는다. 재생과 관련된 설정(WiFi 전용·반복)은 `PlayerViewModel`에 남는다. 재생 화면에서 바꾸고 재생에 곧바로 적용되는 것들이다.
- 근거: [Material 3 색 체계](https://m3.material.io/styles/color/roles), [edge-to-edge와 시스템 표시줄](https://developer.android.com/develop/ui/views/layout/edge-to-edge).

### ADR-066 — 홈과 라이브러리를 가르는 기준 (KM-151)

- PRD 35는 홈에 최근 재생·즐겨찾기·재생목록 세 구획을 두라고 정하고 라이브러리의 내용은 정하지 않았다. KM-112·116에서 그 세 구획을 라이브러리 탭에 만들었으므로, 같은 모양으로 홈을 또 만들면 탭 하나가 남는다.
- **가르는 기준: 홈은 한 번 눌러 다시 듣는 자리, 라이브러리는 전부 보고 정리하는 자리.** PRD가 정한 것은 홈에 무엇이 있어야 하는지이고 어떤 모양인지는 정하지 않았으므로 모양으로 가른다.
- 홈은 가로로 흐르는 카드(이미지가 큰 정사각형)를 쓴다. 세로 목록보다 한 화면에서 여러 구획을 볼 수 있고, 앨범 이미지가 커서 무엇인지 눈으로 고른다. 라이브러리는 세로 목록을 그대로 둔다. 훑어보고 고르는 데는 한 줄에 한 곡이 낫다.
- **홈에는 정리 버튼을 두지 않는다.** 지우기·해제·이름 바꾸기는 모두 라이브러리와 그 아래 화면에 있다. 다시 듣기 위해 들어온 자리에서 실수로 지우는 일이 없어야 한다. 요약 화면에 줄마다 버튼을 두지 않기로 한 것과 같은 판단이다(ADR-059·062).
- **홈에서는 비어 있는 구획을 그리지 않는다.** 라이브러리는 "없습니다"를 말해 주지만(무엇이 없는지 아는 것이 정리의 시작이다) 홈에서 그것을 읽을 이유가 없다. 세 구획이 다 비면 한 줄 안내와 검색으로 가는 버튼만 둔다.
- 카드 줄의 개수를 자르지 않는다. 가로 줄은 보이는 것만 만들므로 길어도 값이 들지 않고, 자르면 누른 자리와 대기열의 자리가 어긋날 위험만 생긴다. 라이브러리 요약이 다섯 개로 자르는 것은 세로로 쌓여 아래 구획을 밀어내기 때문이다(ADR-062).
- 카드를 누르면 그 줄 전체가 대기열이 되고 누른 곡부터 재생한다. 검색 결과·라이브러리와 같다. 재생목록 카드는 재생하지 않고 그 화면을 연다. 곡이 아니라 곡의 묶음이므로 무엇이 담겼는지 보고 고르는 것이 먼저다.
- **시작 목적지를 검색에서 홈으로 옮겼다.** 홈이 자리표시자일 때는 검색이 시작 화면이어야 했다. 이제 앱을 열면 듣던 것이 먼저 보이고 검색은 한 번 눌러 간다. 자리표시자 화면(`NotReadyScreen`)과 그 문자열은 지웠다.
- 새 데이터 경로를 만들지 않았다. 홈은 라이브러리와 같은 `LibraryViewModel`을 본다. Activity 범위라 탭을 옮겨도 같은 인스턴스이며 질의가 늘지 않는다. 그래서 이 작업에는 새 단위 검사가 없다. 새로 판단하는 규칙이 없고 기존 상태를 다른 모양으로 그리기만 한다.
- 근거: PRD 35(Navigation), [LazyRow](https://developer.android.com/develop/ui/compose/lists).
